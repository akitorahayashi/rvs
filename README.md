...
The current MVP turns caption block JSON into narration MP3 files, derives SRT
timing from the generated audio, and renders one captioned MP4 from a project
directory.
...
`file_name`/`text` subtitle and narration blocks. `audio/` is regenerated by
`tts` and ignored by git.
`captions.srt` is regenerated by `srt` and consumed by `render`.
...
SRT parsing, scheduling, formatting, and frame timing live under
`src/rvs/subtitles/`.
